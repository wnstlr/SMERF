...
def original_textcolor_data(n=10000, save=True, save_dir='data'):
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    fname = os.path.join(save_dir, 'textcolor_original.npz')
    ...
    train_data = TextColorDataset(im, labels)
    test_data = TextColorDataset(im_test, labels_test)

    if save:
        np.savez(open(fname, 'wb'), x_train=train_data.X, y_train=train_data.y, x_test=test_data.X, y_test=test_data.y)
        np.savez(open(os.path.join(save_dir, 'textcolor_original_meta.npz'), 'wb'), rep=rep, rep_test=rep_test)
